...
def test_fista_param_markov_step():
    ...
    def prox_map1(x: Var) -> Var:
        x1, x2 = x
        return x1 * 0.5, x2
    
    def prox_map2(x: Var) -> Var:
        x1, x2 = x
        return x1 * u1, x2
    ...
    def prox_map1_prm(x: Var) -> Var:
        x1, x2 = x
        return x1 * 0.5, x2
    
    def prox_map2_prm(x: Var, u1: torch.Tensor) -> Var:
        x1, x2 = x
        return x1 * u1, x2
    ...
    z_curr = PairVar(x_curr, x_prev)
    u = ParamBundle((u1, u2))
    for _ in range(iters):
        z_curr = pm_step1.markov_step(z_curr, u)
    x1_new_pm, x2_new_pm = z_curr.first.data
    ...
    x_prev = x1.clone(), x2.clone() 
    z_curr = PairVar(x_curr, x_prev)
    u = ParamBundle((u1, u2), indices={"grad": "all", "prox": (0)})
    for _ in range(iters):
...
def test_gradient_descent_vjp():
    ...
    assert torch.allclose(grad_x, grad_x_pm.data)
...
def test_implicit_differentiation_with_polyak_step():
    ...
    grad_xm = FlatVar(torch.randn(N))
    u = ParamBundle((A, b, r))
    u_tan = ParamBundle((A_tan, b_tan, r_tan))
    xm = FlatVar(minimizer(A, b, r))
    
    grad_u_anl = torch.func.vjp(minimizer, *u.data)[1](grad_xm.data)
    xm_tan_anl = torch.func.jvp(minimizer, u.data, u_tan.data)[1]
...
def test_implicit_differentiation_with_GD_step():
    ...
    assert torch.allclose(xm_tan_anl, xm_tan_neu.data, atol=1e-4)
